...
TRIGGERS = ["写文件", "保存文件", "写入文件"]

SKILL_INFO = {
    "name": "mcp_fs_write",
    "description": "通过 MCP 写入文件",
    "triggers": TRIGGERS,
    "action": "mcp_fs_write",
    "parameters": {
        "type": "object",
...
def execute(params: str) -> str:
    ...
    if not path or content is None:
        return "请提供 JSON 参数: {\"path\": " + '"/path/file"' + ", \"content\": " + '"内容"' + "}"
    ...
    try:
        result = _call_mcp_tool(command, "write_file", {"path": path, "content": content})
